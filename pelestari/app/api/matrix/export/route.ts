...
// Fetch gambar langsung di server (Bebas CORS!)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const titleLines = [
      ...
      `TANGGAL PELATIHAN ${namaBatchTitle} ${lokasiTitle}`.trim(),
    ];
    ...
    for (let idx = 0; idx < filteredData.length; idx++) {
      ...
      values.forEach((val, cIdx) => {
        ...
        cell.value = val;
        cell.alignment = { vertical: "middle", horizontal: [1, 3, 4, 7].includes(cIdx) ? "left" : "center" };
      });
      dataRow.height = IMAGE_ROW_HEIGHT_PT;

      // Ambil gambar di server
      ...
      if (imgKtp) {
        const id = workbook.addImage({ buffer: imgKtp.buffer as any, extension: imgKtp.extension });
        sheet.addImage(id, {
      ...
      if (imgSim) {
        const id = workbook.addImage({ buffer: imgSim.buffer as any, extension: imgSim.extension });
        sheet.addImage(id, {
      ...
      if (imgPas) {
        const id = workbook.addImage({ buffer: imgPas.buffer as any, extension: imgPas.extension });
        sheet.addImage(id, {
