...
export default function PemohonPage() {
  ...
  const filteredList = useMemo(() => {
    const result = list
      ...
      .sort((a, b) => a.nama_pemohon.localeCompare(b.nama_pemohon))
  ...
  const handleSave = async () => {
    ...
    let res
    if (editId) {
      res = await updatePemohon(editId, input)
    } else {
      res = await createPemohon(input)
    }
    if (res && res.success) {
      swal.success(editId ? "Pemohon berhasil diperbarui" : "Pemohon berhasil ditambahkan")
      setInput("")
      setEditId(null)
      await refreshData()
        ...
        <Card className="border shadow-sm bg-zinc-50/50 rounded-md">
          <CardContent className="p-4 flex gap-3 items-end">
            <div className="flex-1 space-y-1.5">
              <label className="text-[10px] font-black uppercase italic text-zinc-500 ml-1">Nama Pemohon</label>
              <Input
                placeholder="Tambah/Edit nama pemohon..."
                value={input}
          ...
          <Table>
            <TableHeader className="bg-zinc-50">
              <TableRow className="hover:bg-zinc-50 border-b border-zinc-300">
                ...
                <TableHead className="text-zinc-800 font-black uppercase text-[10px] tracking-widest">Nama Pemohon</TableHead>
                  ...
                  <TableRow key={item.id} className="group border-b border-zinc-100 last:border-0 hover:bg-zinc-50/50 transition-colors">
                    ...
                    <TableCell className="py-3">
                      <div className="flex justify-center gap-1">
                        <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-zinc-200 rounded-sm" onClick={() => { setEditId(item.id); setInput(item.nama_pemohon); }}>
                          <Edit2 className="h-3.5 w-3.5 text-zinc-600" />
                        </Button>
                        <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-red-50 hover:text-red-600 rounded-sm" onClick={async () => { if (await swal.confirm(`Hapus data?`)) { const res = await deletePemohon(item.id); if (res && res.success) { swal.success("Pemohon berhasil dihapus"); await refreshData(); } else { swal.error(res?.message || "Gagal menghapus data"); } } }}>
