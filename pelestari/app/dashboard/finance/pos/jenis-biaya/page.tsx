...
import { Trash2, Edit2, Plus, Save, X, Search, SortAsc } from "lucide-react"
// Pastikan kamu sudah membuat file actions ini
import { 
  getJenisBiaya, 
  createJenisBiaya, 
  updateJenisBiaya, 
...
export default function JenisBiayaPage() {
  ...
  const refreshData = async () => {
    const data = await getJenisBiaya()
    setList(data)
  }

  useEffect(() => { refreshData() }, [])

  // Filter & Sort A-Z berdasarkan field jenis_biaya
  const filteredList = useMemo(() => {
    const result = list
      .filter((item) =>
        item.jenis_biaya.toString().toLowerCase().includes(searchQuery.toLowerCase())
      )
      .sort((a, b) => a.jenis_biaya.toString().localeCompare(b.jenis_biaya.toString()))
  ...
  const handleSave = async () => {
    ...
    setLoading(true)
    if (editId) {
      await updateJenisBiaya(editId, input)
    } else {
      await createJenisBiaya(input)
    }
    setInput("")
    setEditId(null)
    await refreshData()
  ...
  return (
    <div className="p-6 w-full space-y-6 font-sans text-zinc-900">
      ...
      <div className="flex items-center justify-between border-b border-zinc-200 pb-4">
        <div>
          <h1 className="text-2xl font-black uppercase tracking-tighter italic text-black">Master Jenis Biaya</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border shadow-sm bg-zinc-50/50 rounded-md">
          <CardContent className="p-4 flex gap-3 items-end">
            <div className="flex-1 space-y-1.5">
              <label className="text-[10px] font-black uppercase italic text-zinc-500 ml-1">Nama Jenis Biaya</label>
              <Input 
                placeholder="Tambah/Edit jenis biaya..." 
                value={input} 
        ...
        <Card className="border shadow-sm bg-white rounded-md">
          <CardContent className="p-4 flex gap-3 items-end">
            <div className="flex-1 space-y-1.5">
              <label className="text-[10px] font-black uppercase italic text-zinc-500 ml-1">Cari Jenis Biaya</label>
          ...
          <Table>
            <TableHeader className="bg-zinc-50">
              <TableRow className="hover:bg-zinc-50 border-b border-zinc-300">
                ...
                <TableHead className="text-zinc-800 font-black uppercase text-[10px] tracking-widest">Jenis Biaya</TableHead>
