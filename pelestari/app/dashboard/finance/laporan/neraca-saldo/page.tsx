...
import { getNeracaSaldo, NeracaSaldoItem } from "@/app/actions/neraca-saldo"
...
export default function NeracaSaldoPage() {
  ...
  async function loadData(
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || "Gagal mengambil data neraca saldo")
  ...
  const filteredData = data.filter(
    (item) => Number(item.debit || 0) !== 0 || Number(item.kredit || 0) !== 0
  )
  ...
  return (
    <div className="w-full space-y-6 p-6 font-sans text-zinc-900">
        ...
        <CardContent className="px-6 py-5">
          <div className="flex flex-col gap-4 md:flex-row md:items-end">
            ...
            <Button
              ...
              className="h-10 gap-2 rounded-sm bg-zinc-900 px-5 text-xs font-bold text-white hover:bg-zinc-800"
            >
      ...
      <div className="flex justify-between px-2 text-[9px] font-bold tracking-wider text-zinc-400 uppercase">
        <p>* Sistem Neraca Saldo Terintegrasi</p>
      </div>
    </div>
  )
}
